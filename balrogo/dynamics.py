...
def v_sky_to_polar(a,d,pma,pmd,a0,d0,pma0,pmd0) :
    ...
    dmuy  = (cosd * cosd0 + sind * sind0 * cosda) * pmd - \
            cost * pmd0 + (pma - pma0 * cosd / cosd0) * sind0 * sinda
...
def unc_sky_to_polar(a,d,epma,epmd,epmad,a0,d0,epma0,epmd0):
    ...
    dvdpmd  = (-cosd * sinda**2 * sind
              + (cosd0 * sind - cosda * cosd * sind0) \
              * (cosd* cosd0 + cosda * sind * sind0)) / dentheta
    dvdpma0 = - cosd * sinda * (cosda * cosd \
              * cosd0 + sind * sind0) / dentheta 
    dvdpmd0 = - (cosd0 * sind - cosda * cosd * sind0) \
              * (cosda * cosd * cosd0 + sind * sind0) / dentheta
    
    uncpmr  = np.sqrt((dvdpma * epma)**2 + (dvdpmd * epmd)**2
              + (dvdpma0 * epma0)**2 + (dvdpmd0 * epmd0)**2 
              + 2 * (dvdpma * dvdpmd * epma * epmd * epmad))
    

    dvdpma  = (cosa * cosa0 * cosd0 * sind + cosd0 * sina * sina0 * sind \
              - cosd * sind0) / dentheta
    dvdpmd  = (- cosd0 * sinda)  / dentheta 
    dvdpma0 = cosd * (-cosa * cosa0 * sind - sina * sina0 * sind \
              + cosd * sind0/cosd0) / dentheta
    dvdpmd0 = cosd * sinda * (cosda * cosd * cosd0 + sind * sind0) / dentheta

    uncpmt  = np.sqrt((dvdpma * epma)**2 + (dvdpmd * epmd)**2
              + (dvdpma0 * epma0)**2 + (dvdpmd0 * epmd0)**2 
              + 2 * (dvdpma * dvdpmd * epma * epmd * epmad))
...
def aux_disp(idx,y,ey,dimy,robust_sig):
    ...
    if (robust_sig is True) :
        ...
        for i in range(0,dimy) :
            ...
            if (len(idx) < 30) :
                disp[i,0] = np.sqrt(np.nanstd(y[i][idx],ddof=1)**2 - np.nanmean(ey[i][idx]**2))
            else :
                disp[i,0] = np.sqrt(np.nanstd(y[i][idx],ddof=0)**2 - np.nanmean(ey[i][idx]**2))
...
def bin_disp1d(x,y,ey,dimy,bins,bootp=True,logx=True):
    ...
    for i in range(0,dimy):
        for j in range (0, bins-1) :
            ...
            disp[i,j] = np.sqrt(np.nanstd(y[i][cond],ddof=dof)**2 - np.nanmean(ey[i][cond]**2))
...
def moving_grid1d(x,y,ey,dimy,bootp=True,logx=True,bins=10,ngrid=10):
    ...
    for i in range(1,ngrid) :
        ...
        if (i == 1) :
            r, disp, err = bin_disp1d(xi,yi,eyi,dimy,bins=bins,bootp=bootp,logx=logx)
        else:
            ri, dispi, erri = bin_disp1d(xi,yi,eyi,dimy,bins=bins,bootp=bootp,logx=logx)
...
def equal_size(x,y,ey,dimy,bootp=True,logx=True,nbin=10):
    ...
    for i in range (0, size) :
        ...
        for j in range(0,dimy) :
            ...
            disp[j,i] = np.sqrt(np.nanstd(y[j][nbin*i:nbin*(i+1)-1],ddof=dof)**2 \
                                - np.nanmean(ey[j][nbin*i:nbin*(i+1)-1]**2))
        
            if (bootp == True) :
                err[j,i] = bootstrap(y[j][nbin*i:nbin*(i+1)-1])
            else :
                err[j,i] = disp[j,i] / np.sqrt(2 * \
                (len(y[j][nbin*i:nbin*(i+1)-1][np.logical_not(np.isnan(y[j][nbin * i:nbin*(i+1)-1]))])-1))
...
def perc_bins(x,y,ey,dimy,bootp=True,logx=True,nnodes=5):
    ...
    for i in range(0,nnodes) :
        ...
        for j in range(0,dimy) :
            ...
            disp[j,i] = np.sqrt(np.nanstd(y[j][idxr],ddof=dof)**2 - np.nanmean(ey[j][idxr]**2))
            
            if (bootp == True) :
                err[j,i] = bootstrap(y[j][idxr])
            else :
                err[j,i] = disp[j,i] / np.sqrt(2 * \
                (len(y[j][idxr][np.logical_not(np.isnan(y[j][idxr]))])-1))
...
def disp1d(x,y,ey,dimy,bins="percentile",smooth=True,bootp=True,logx=True,
           nbin=None,polorder=None,return_fits=False):
    """
    ...
    if bins == "moving":
        if nbin is None:
            ...
            bins = nbin
        ngrid = 2
        r, disp, err = moving_grid1d(x, y, ey, dimy,bootp=bootp,logx=logx,
...
def disp2d(x,y,ey,dimy,smooth=True,bootp=True,
           a0=None,d0=None,robust_sig=False,nbin=None,nmov=None):
    """
    ...
    raux_err  = partial(aux_err,y=y,ey=ey,dimy=dimy,robust_sig=robust_sig,bootp=bootp)
    
    for i in range(0,len(amin)) :
        
        index = np.arange(len(x[0]))
        
        hex_disp  = plt.hexbin(x[0],x[1],C=index,
                    gridsize=nbin,reduce_C_function=raux_disp,
        ...
        hex_err  = plt.hexbin(x[0],x[1],C=index,
                   gridsize=nbin,reduce_C_function=raux_err,
                    extent=(amin[i],amax[i],dmin[i],dmax[i]))
    ...
    grid_x, grid_y = np.mgrid[np.amin(alim0):np.amax(alim0):300j, 
                              np.amin(dlim0):np.amax(dlim0):300j]
...
def dispersion(x,y,ey,bins=None,smooth=True,bootp=False,logx=False,nbin=None,polorder=None,
               return_fits=False,robust_sig=False,a0=None,d0=None,nmov=None) :
    """
    ...
    if (dimx == 1):
        r, disp, err = disp1d(x,y,ey,dimy,bins=bins,smooth=smooth,bootp=bootp,
                              polorder=polorder,
                              logx=logx,nbin=nbin,return_fits=return_fits)
    else :
        r, disp, err = disp2d(x,y,ey,dimy,smooth=smooth,bootp=bootp,
                              a0=a0,d0=d0,robust_sig=robust_sig,
                              nbin=nbin,nmov=nmov)
...
def plot_disp1D(ra,dec,pmra,epmra,pmdec,epmdec,corrpm,
                ra0,dec0,pmra0,epmra0,pmdec0,epmdec0,vlos0,d0,
                logx=False,logy=False,
                bootp=False,polorder=None,return_fits=False):
    """
    ...
    uncpmr, uncpmt = unc_sky_to_polar(ra,dec, epmra, epmdec, corrpm, ra0, dec0, epmra0, epmdec0)
    ...
    rr, dd, err = dispersion(rproj,[pmr,pmt],[uncpmr,uncpmt],
                             bins="percentile",polorder=polorder,
                             bootp=bootp,logx=logx,
                             return_fits=return_fits)
    
    rr2, dd2, err2 = dispersion(rproj,[pmr,pmt],[uncpmr,uncpmt],
                                bins=int(0.5*position.good_bin(rproj)),
                                bootp=False,logx=True,smooth=False)
    ...
    plt.errorbar(rr2,dd2,yerr = err2, color = "b", 
                  ls = 'none', barsabove = True, zorder=10)
...
def plot_disp2D(ra,dec,pmra,epmra,pmdec,epmdec,corrpm,
                ra0,dec0,pmra0,epmra0,pmdec0,epmdec0,vlos0,d0,
                rlim=None,bootp=False,robust_sig=False,nbin=None,nmov=None,
                return_fits=False):
    """
    ...
    rr, dd, err = dispersion([ra,dec],[pmr,pmt],[uncpmr,uncpmt],
                             bootp=bootp,a0=ra0,d0=dec0,
                             robust_sig=robust_sig,nbin=nbin,nmov=nmov)
    ...
    c=plt.imshow(dd.T,origin='lower',aspect='auto',vmin=qmin,vmax=qmax,cmap='jet',
    ...
    c=plt.imshow(err.T,origin='lower',aspect='auto',vmin=qmin,vmax=qmax,cmap='jet',
                 extent=[np.nanmin(alim0),np.nanmax(alim0),np.nanmin(dlim0),np.nanmax(dlim0)])
    cbar = fig.colorbar(c)
    cbar.ax.tick_params(labelsize=13) 
    plt.xlim([np.amin(xlim),np.amax(xlim)])
